...
import { db, hashPassword, User } from "./src/server/db";
...
app.post("/api/auth/register", (req, res) => {
  try {
    ...
    const existing = db.getUsers().find(u => u.email === emailClean);
    if (existing) {
    ...
    let org = db.getOrganizations().find(o => o.domain === domain);
    let role: "Admin" | "User" = "User";

    if (!org) {
      // First user registering under this domain becomes the Admin
      org = db.createOrganization(organizationName || `${domain.split(".")[0].toUpperCase()} Workspace`, domain);
      role = "Admin";
    }
...
app.post("/api/auth/login", (req, res) => {
  try {
    ...
    const emailClean = email.toLowerCase().trim();
    const user = db.getUsers().find(u => u.email === emailClean);
    if (!user) {
    ...
    const org = db.getOrganizations().find(o => o.id === user.organization_id);
...
app.post("/api/auth/edit-profile", authenticateToken, (req, res) => {
  try {
    const user = (req as any).user;
...
// INVITATIONS
app.post("/api/auth/invite", authenticateToken, (req, res) => {
  try {
    const user = (req as any).user;
    if (user.role !== "Admin") {
    ...
    const invitation = db.createInvitation(user.organization_id, emailClean, role);
...
app.get("/api/auth/invitation/:token", (req, res) => {
  try {
    const invite = db.getInvitations().find(i => i.invitation_token === req.params.token && !i.accepted);
    if (!invite) {
      res.status(400).json({ success: false, error: "Invalid, expired, or accepted invitation." });
      return;
    }

    const org = db.getOrganizations().find(o => o.id === invite.organization_id);
...
app.post("/api/auth/accept-invitation", (req, res) => {
  try {
    ...
    const newUser = db.acceptInvitation(token, password, fullName);
    const org = db.getOrganizations().find(o => o.id === newUser.organization_id);
...
app.get("/api/admin/users", authenticateToken, (req, res) => {
  try {
    ...
    const orgUsers = db.getUsers().filter(u => u.organization_id === user.organization_id);
...
app.post("/api/admin/users/:id/role", authenticateToken, (req, res) => {
  try {
    ...
    const targetUser = db.getUsers().find(u => u.id === targetUserId && u.organization_id === adminUser.organization_id);
    if (!targetUser) {
...
app.post("/api/admin/users/:id/status", authenticateToken, (req, res) => {
  try {
    ...
    db.updateUser(targetUserId, { status });
...
app.post("/api/admin/users/:id/resend-invite", authenticateToken, (req, res) => {
  try {
    const adminUser = (req as any).user;
    if (adminUser.role !== "Admin") {
      res.status(403).json({ success: false, error: "Access Denied." });
    ...
    const invitation = db.createInvitation(adminUser.organization_id, targetUser.email, targetUser.role);
    const signupLink = `/invite?token=${invitation.invitation_token}`;
...
app.get("/api/business/customers", authenticateToken, (req, res) => {
  const user = (req as any).user;
  res.json({ success: true, data: db.getCustomers(user.organization_id) });
});

app.post("/api/business/customers", authenticateToken, (req, res) => {
  const user = (req as any).user;
  const saved = db.saveCustomer(user.organization_id, req.body, user.id);
  res.json({ success: true, data: saved });
});

app.delete("/api/business/customers/:id", authenticateToken, (req, res) => {
  const user = (req as any).user;
  db.deleteCustomer(user.organization_id, req.params.id);
  res.json({ success: true });
});

// 1.1 Yamazumi Studies (Per Customer)
app.get("/api/business/yamazumi-studies", authenticateToken, (req, res) => {
  const user = (req as any).user;
  const customerId = req.query.customerId as string;
  res.json({ success: true, data: db.getYamazumiStudies(user.organization_id, customerId) });
});

app.post("/api/business/yamazumi-studies", authenticateToken, (req, res) => {
  const user = (req as any).user;
  const saved = db.saveYamazumiStudy(user.organization_id, req.body, user.id);
  res.json({ success: true, data: saved });
});

app.delete("/api/business/yamazumi-studies/:id", authenticateToken, (req, res) => {
  const user = (req as any).user;
  db.deleteYamazumiStudy(user.organization_id, req.params.id);
  res.json({ success: true });
});

// 2. Processes
app.get("/api/business/processes", authenticateToken, (req, res) => {
  const user = (req as any).user;
  const factoryId = req.headers["x-factory-id"] as string;
  res.json({ success: true, data: db.getProcesses(user.organization_id, factoryId) });
});

app.post("/api/business/processes", authenticateToken, (req, res) => {
  const user = (req as any).user;
  const factoryId = req.headers["x-factory-id"] as string;
  const payload = { ...req.body };
  if (factoryId && !payload.factory_id) {
    payload.factory_id = factoryId;
  }
  const saved = db.saveProcess(user.organization_id, payload, user.id);
  res.json({ success: true, data: saved });
});

app.delete("/api/business/processes/:id", authenticateToken, (req, res) => {
  const user = (req as any).user;
  db.deleteProcess(user.organization_id, req.params.id);
  res.json({ success: true });
});

// 3. Gantt Activities (Timeline)
app.get("/api/business/gantt", authenticateToken, (req, res) => {
  const user = (req as any).user;
  const factoryId = req.headers["x-factory-id"] as string;
  res.json({ success: true, data: db.getActivities(user.organization_id, factoryId) });
});

app.post("/api/business/gantt", authenticateToken, (req, res) => {
  const user = (req as any).user;
  const factoryId = req.headers["x-factory-id"] as string;
  const payload = { ...req.body };
  if (factoryId && !payload.factory_id) {
  ...
  const saved = db.saveActivity(user.organization_id, payload, user.id);
  res.json({ success: true, data: saved });
});

app.delete("/api/business/gantt/:id", authenticateToken, (req, res) => {
  const user = (req as any).user;
  db.deleteActivity(user.organization_id, req.params.id);
  res.json({ success: true });
});

// 4. Spaghetti Flow Segments
app.get("/api/business/segments", authenticateToken, (req, res) => {
  const user = (req as any).user;
  const factoryId = req.headers["x-factory-id"] as string;
  res.json({ success: true, data: db.getSegments(user.organization_id, factoryId) });
});

app.post("/api/business/segments", authenticateToken, (req, res) => {
  const user = (req as any).user;
  const factoryId = req.headers["x-factory-id"] as string;
  const payload = { ...req.body };
  if (factoryId && !payload.factory_id) {
  ...
  const saved = db.saveSegment(user.organization_id, payload, user.id);
  res.json({ success: true, data: saved });
});

app.delete("/api/business/segments/:id", authenticateToken, (req, res) => {
  const user = (req as any).user;
  db.deleteSegment(user.organization_id, req.params.id);
  res.json({ success: true });
});

app.post("/api/business/segments/clear", authenticateToken, (req, res) => {
  const user = (req as any).user;
  const factoryId = req.headers["x-factory-id"] as string;
  db.clearSegments(user.organization_id, factoryId);
  res.json({ success: true });
});

// 5. Kaizens (Ideas Board)
app.get("/api/business/kaizens", authenticateToken, (req, res) => {
  const user = (req as any).user;
  const factoryId = req.headers["x-factory-id"] as string;
  res.json({ success: true, data: db.getKaizens(user.organization_id, factoryId) });
});

app.post("/api/business/kaizens", authenticateToken, (req, res) => {
  const user = (req as any).user;
  const factoryId = req.headers["x-factory-id"] as string;
  const payload = { ...req.body };
  if (factoryId && !payload.factory_id) {
  ...
  const saved = db.saveKaizen(user.organization_id, payload, user.id);
  res.json({ success: true, data: saved });
});

app.delete("/api/business/kaizens/:id", authenticateToken, (req, res) => {
  const user = (req as any).user;
  db.deleteKaizen(user.organization_id, req.params.id);
  res.json({ success: true });
});

// 6. Audits (5S)
app.get("/api/business/audits", authenticateToken, (req, res) => {
  const user = (req as any).user;
  const factoryId = req.headers["x-factory-id"] as string;
  res.json({ success: true, data: db.getAudits(user.organization_id, factoryId) });
});

app.post("/api/business/audits", authenticateToken, (req, res) => {
  const user = (req as any).user;
  const factoryId = req.headers["x-factory-id"] as string;
  const payload = { ...req.body };
  if (factoryId && !payload.factory_id) {
  ...
  const saved = db.saveAudit(user.organization_id, payload, user.id);
  res.json({ success: true, data: saved });
});

app.delete("/api/business/audits/:id", authenticateToken, (req, res) => {
  const user = (req as any).user;
  db.deleteAudit(user.organization_id, req.params.id);
  res.json({ success: true });
});

// 7. VSM Projects
app.get("/api/business/vsm-projects", authenticateToken, (req, res) => {
  const user = (req as any).user;
  const factoryId = req.headers["x-factory-id"] as string;
  res.json({ success: true, data: db.getVsmProjects(user.organization_id, factoryId) });
});

app.post("/api/business/vsm-projects", authenticateToken, (req, res) => {
  const user = (req as any).user;
  const factoryId = req.headers["x-factory-id"] as string;
  const payload = { ...req.body };
  if (factoryId && !payload.factory_id) {
  ...
  const saved = db.saveVsmProject(user.organization_id, payload, user.id);
  res.json({ success: true, data: saved });
});

app.delete("/api/business/vsm-projects/:id", authenticateToken, (req, res) => {
  const user = (req as any).user;
  db.deleteVsmProject(user.organization_id, req.params.id);
  res.json({ success: true });
});


// 8. OpEx Assessments
app.get("/api/business/opex-assessments", authenticateToken, (req, res) => {
  const user = (req as any).user;
  const customerId = req.headers["x-factory-id"] as string;
  res.json({ success: true, data: db.getOpexAssessments(user.organization_id, customerId) });
});

app.post("/api/business/opex-assessments", authenticateToken, (req, res) => {
  const user = (req as any).user;
  const customerId = req.headers["x-factory-id"] as string;
  const payload = { ...req.body };
  ...
  const saved = db.saveOpexAssessment(user.organization_id, payload, user.id);
  res.json({ success: true, data: saved });
});

app.delete("/api/business/opex-assessments/:id", authenticateToken, (req, res) => {
  const user = (req as any).user;
  db.deleteOpexAssessment(user.organization_id, req.params.id);
  res.json({ success: true });
});
...
app.post("/api/gemini/yamazumi-analyze", authenticateToken, async (req, res) => {
  try {
    const { elements, stats, language } = req.body;
    const client = getGeminiClient();
...
Analyze the following process study elements and statistics gathered from video analysis:
...
Please provide a highly structured, professional, dark-themed industrial software style markdown report with exactly these sections. Avoid any conversational greeting, play right into the facts:

### 1. Bottleneck & Variation Analysis
- Identify the bottleneck element, discuss its standard cycle time vs the average, describe the motion waste or overprocessing, and identify periodic/repeat losses.

### 2. COPQ Opportunity Loss Calculation
...
### 3. Priority Lean Recommendations (High / Medium / Low Impact)
- Prioritize at least 3 concrete, actionable mechanical/work-cell recommendations. Follow the exact style specified by the user:
  ...
  - "Waiting losses represent W% of total cycle time."
...
app.post("/api/gemini/yamazumi-chat", authenticateToken, async (req, res) => {
  try {
    const { message, history, elements, stats, language } = req.body;
    const client = getGeminiClient();
...
You are a Yamazumi AI Lean Manufacturing Assistant inside a professional industrial motion study and work-balancing software.
