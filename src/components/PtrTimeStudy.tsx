...
// accepts the legacy HTML 1-7 scale, never real px; these codes are translated to real px (via
// FONT_SIZE_PX_BY_LEGACY_CODE, read off the <font size="N"> execCommand actually inserts) so the
// editor's own DOM uses a real CSS font-size instead of that legacy attribute.
...
// One formatted text run extracted from the note editor's DOM — see extractNoteRuns. The server
// (buildNoteHtmlFromRuns in app.ts) builds the actual HTML from these; the client never sends raw
// innerHTML, so there's no attacker-controlled markup for the server to parse/sanitize, just data
// to validate (color/fontSize are checked there too — this type is the wire contract, not a
...
const NOTE_BLOCK_TAGS = new Set(["div", "p"]);

// Walks the contentEditable note editor's DOM and flattens it into an ordered list of runs. Bold/
// italic/underline/span(color|font-size) are the only wrapper elements the toolbar below ever
// produces (execCommand output is normalized to these via normalizeLegacyFontTags), so a plain
// recursive walk carrying accumulated formatting down through each level is sufficient. <br> becomes
// an explicit break run; consecutive block-level children (some browsers wrap a new line in a <div>
// on Enter instead of inserting <br>) are also separated with a break run as a defensive fallback.
function extractNoteRuns(root: HTMLElement): NoteRun[] {
  const runs: NoteRun[] = [];
  const walkChildren = (parent: HTMLElement | DocumentFragment, fmt: Omit<NoteRun, "text" | "break">) => {
    let sawBlock = false;
    parent.childNodes.forEach(child => {
      const isBlockEl = child.nodeType === Node.ELEMENT_NODE && NOTE_BLOCK_TAGS.has((child as HTMLElement).tagName.toLowerCase());
      if (isBlockEl) {
        if (sawBlock) runs.push({ break: true });
        sawBlock = true;
      }
      walk(child, fmt);
    });
  };
  const walk = (node: ChildNode, fmt: Omit<NoteRun, "text" | "break">) => {
    ...
    const nextFmt: Omit<NoteRun, "text" | "break"> = { ...fmt };
    if (tag === "b" || tag === "strong") nextFmt.bold = true;
    if (tag === "i" || tag === "em") nextFmt.italic = true;
    if (tag === "u") nextFmt.underline = true;
    if (tag === "span") {
      if (el.style.color) nextFmt.color = el.style.color;
      const px = parseFloat(el.style.fontSize || "");
      if (!isNaN(px)) nextFmt.fontSize = px;
    }
    walkChildren(el, nextFmt);
  };
  walkChildren(root, {});
  return runs;
}
...
export default function PtrTimeStudy({ activities, onAddActivity, onUpdateActivity, onAddKaizen, kaizens }: PtrTimeStudyProps) {
  ...
  // on the backend). `weeklyNotes` holds everyone's notes for the reported week. The note itself is
  // basic-rich-text (bold/italic/underline/font-size/color — see the contentEditable editor + toolbar
  // below); its HTML is only ever written into the DOM imperatively (on load, via noteEditorRef), so
  // React re-renders never fight the browser's own cursor position inside the editable div.
  ...
  useEffect(() => {
    ...
    fetch(`/api/business/weekly-consultant-notes?week=${reportWeekNum}&year=${reportYear}`, {
      ...
      .then(res => {
        ...
        if (noteEditorRef.current) noteEditorRef.current.innerHTML = mine?.note || "";
      })
      .catch(err => {
  ...
  // document.execCommand (still supported everywhere despite the formal deprecation; a real
  ...
  const applyBold = () => { noteEditorRef.current?.focus(); document.execCommand("bold"); normalizeLegacyFontTags(); };
  const applyItalic = () => { noteEditorRef.current?.focus(); document.execCommand("italic"); normalizeLegacyFontTags(); };
  const applyUnderline = () => { noteEditorRef.current?.focus(); document.execCommand("underline"); normalizeLegacyFontTags(); };
  ...
  const saveMyWeeklyNote = () => {
    ...
    const customerId = selectedCustomer?.id || "default";
    // The editor's own DOM is the only source of truth for the current note — extracted into
    // "runs" here (never raw HTML, see extractNoteRuns/buildNoteHtmlFromRuns in app.ts).
    const runs = noteEditorRef.current ? extractNoteRuns(noteEditorRef.current) : [];
    ...
    fetch("/api/business/weekly-consultant-notes", {
      ...
      body: JSON.stringify({ week: reportWeekNum, year: reportYear, runs })
    })
      .then(res => res.json())
      .then(res => {
